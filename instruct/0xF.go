...
func (cpu *Chip8) opcodeFx07(x uint16) {
...
func (cpu *Chip8) OpcodeFx0A(x uint16) {
	...
	for i := range cpu.Keys {
		if cpu.Keys[i] {
			cpu.register[secondDigit] = byte(i + 1)
		}
	}
}

func (cpu *Chip8) opcodeFx15(x uint16) {
	/*Set delay timer = Vx.*/
	cpu.DelayTimer = cpu.register[x]
}

func (cpu *Chip8) opcodeFx18(x uint16) {
	/*Set sound timer = Vx*/
...
func (cpu *Chip8) opcodeFx29(x uint16) {
	fmt.Printf("COMMENCEMENT : %02x \n", x)

	/*Set I = location of sprite for digit Vx.*/
	cpu.I = uint16(cpu.register[x])
}
